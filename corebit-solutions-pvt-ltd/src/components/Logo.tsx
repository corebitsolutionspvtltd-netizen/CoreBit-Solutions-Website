...
export function LogoIcon({ size = "md", className = "" }: { size?: "sm" | "md" | "lg" | "xl"; className?: string }) {
  ...
  return (
    <div 
      className={`relative shrink-0 flex items-center justify-center font-sans font-black tracking-tighter select-none bg-gradient-to-b from-slate-900 to-slate-950 border border-white/10 shadow-lg shadow-purple-500/10 ${sizeClasses[size]} ${className}`}
      id="brand-cb-badge"
    >
      {/* Decorative gradient soft radial glow */}
      <div className="absolute inset-0 bg-gradient-to-tr from-purple-500/5 via-transparent to-orange-500/5 opacity-60 rounded-inherit" />

      {/* Styled CB typography with premium look */}
      ...
      {/* Perfect tech-accent dots */}
      <div className="absolute top-1 right-1 w-1 h-1 bg-purple-500/30 rounded-full animate-pulse" />
      <div className="absolute bottom-1 left-1 w-1 h-1 bg-orange-500/30 rounded-full animate-pulse" />
    </div>
  );
}
