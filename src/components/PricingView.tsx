...
export default function PricingView({
  ...
  // Load plans state from LocalStorage with fallback safety
  const [websitePlans, setWebsitePlans] = useState<FeaturePlan[]>(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Failed to parse website plans:", e);
      }
    }
    return DEFAULT_WEBSITE_PLANS;
  });
  ...
  const [applicationPlans, setApplicationPlans] = useState<FeaturePlan[]>(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Failed to parse app plans:", e);
      }
    }
    return DEFAULT_APP_PLANS;
  });
  ...
  const [hybridPlans, setHybridPlans] = useState<FeaturePlan[]>(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Failed to parse hybrid plans:", e);
      }
    }
    return DEFAULT_BOTH_PLANS;
  });
