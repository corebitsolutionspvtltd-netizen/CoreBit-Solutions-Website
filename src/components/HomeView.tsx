...
export default function HomeView({ onPageChange }: HomeViewProps) {
  ...
  // Load latest data from Firestore on mount and register listeners for real-time remote updates
  useEffect(() => {
    const loadCloudData = async () => {
      try {
        const cloudHome = await fetchHomeSettings();
        if (cloudHome) {
          setHomeTexts({ ...DEFAULT_HOME_TEXTS, ...cloudHome });
        }
    ...
    // Event handlers to update React state immediately when databases change from other website sessions
    const handleHomeSync = () => {
      const saved = localStorage.getItem("corebit_home_settings");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          const merged = { ...DEFAULT_HOME_TEXTS, ...parsed };
          setHomeTexts(prev => JSON.stringify(prev) !== JSON.stringify(merged) ? merged : prev);
        } catch {}
      }
    };

    const handleRolesSync = () => {
      const saved = localStorage.getItem("corebit_roles_data");
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          setRolesData(prev => JSON.stringify(prev) !== JSON.stringify(parsed) ? parsed : prev);
        } catch {}
      }
    };
    ...
    const handleNavSync = () => {
      ...
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          setNavCards(prev => JSON.stringify(prev) !== JSON.stringify(parsed) ? parsed : prev);
        } catch {}
      }
    };

    window.addEventListener("corebit_home_settings_updated", handleHomeSync);
    window.addEventListener("corebit_roles_data_updated", handleRolesSync);
    window.addEventListener("corebit_nav_cards_updated", handleNavSync);

    return () => {
      window.removeEventListener("corebit_home_settings_updated", handleHomeSync);
      window.removeEventListener("corebit_roles_data_updated", handleRolesSync);
      window.removeEventListener("corebit_nav_cards_updated", handleNavSync);
    };
  }, []);
