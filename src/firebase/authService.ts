  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (isAdminEmail(promptEmail)) {
        ...
        safeSessionStorage.setItem("corebit_mock_user", JSON.stringify(mockUser));
        safeStorage.setItem("corebit_admin_mode", "true");
...
export async function logoutUser(): Promise<boolean> {
  ...
  safeSessionStorage.removeItem("corebit_mock_user");
  safeStorage.removeItem("corebit_mock_user");
  safeStorage.removeItem("corebit_admin_mode");
  safeStorage.removeItem("cbit_admin_access");
  window.dispatchEvent(new Event("corebit_admin_mode_changed"));
  return true;
}
...
export function subscribeToAuth(callback: (user: User | null, isAdmin: boolean) => void): () => void {
  ...
  const checkMockUser = (): boolean => {
    const raw = safeSessionStorage.getItem("corebit_mock_user") || safeStorage.getItem("corebit_mock_user");
    if (raw) {
      try {
